...
public class Application_C3 {
    ...
    private void run() {
        ...
        Function<Stream<Customer>, Stream<Customer>> customerStream = customer -> customer.sorted((a, b) -> a.getLastName().compareTo(b.getLastName()));
        Function<Stream<Customer>, Stream<Customer>> customerStream2 = customer -> customer
                .sorted((a, b) -> a.getLastName().compareTo(b.getLastName()))
                .filter(c -> c.getId() >= 400000 && c.getId() < 500000);
        //
        StringBuffer sb = new StringBuffer("Customers:\n");
        printCustomers(sb, customers);
        //
        printCustomers(sb, customers, customerStream);    // Aufgaben 4.), 5.)
        //
        printCustomers(sb, customers, customerStream2);
    ...
    public String fmtCustomerName(final Customer c, int... style) {
        ...
        switch (style.length > 0 ? style[0] : 0) {    // 0 is default format
            ...
            case 5:
                return fn;
            case 6:
                return String.format("%s.%s.", fn1, fmtCustomerName(c, 9));
            case 7:
                return String.format("%s%s", fn1, fmtCustomerName(c, 9));
            case 8:
                return fn1;

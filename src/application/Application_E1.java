...
public class Application_E1 {
    ...
    private Order o6173 = null;        // Lena's order
...
//                .line();
    ...
    private List<Customer> buildCustomers() {
        ...
        var result = toList(eric, anne, tim, nadine, khaled, lena);
    ...
    private void moreOrders() {
        ...
        Article fahrradkette = new Article("Fahrradkette", 695).setTax(TAX.GER_VAT_REDUCED).setId("SKU-583978");
        //
        this.articles.addAll(Arrays.asList(pfanne, fahrradhelm, fahrradkette));

        // Eric's 3rd order
        Order o7356 = new Order(eric)
                .setId("7372561535")
                .addItem(fahrradhelm, 1)
                .addItem(fahrradkette, 1);
        //
        // Eric's 4th order
...
//		return sb;
    ...
    public TablePrinter printOrder(final TablePrinter orderTable, final Order order) {
        ...
        if (orderTable != null && order != null) {
            ...
            var valueAndTax = calculateValueAndTax(order);
            String compoundMwst = fmtPrice(valueAndTax[1], 1);
            String compoundValue = fmtPrice(valueAndTax[0], 1);
            ...
            while (items.hasNext()) {
                ...
                String itemPriceStr = fmtPrice(itemPrice);

                long sumItemPrice = itemPrice * itemAmount;
                String sumItemPriceStr = fmtPrice(sumItemPrice, 1);

                TAX tax = item.getArticle().getTax();

                long includedVAT = calculateIncludedVAT(itemPrice, tax);
                String includedVATStr = fmtPrice(includedVAT);

                long sumIncludedVAT = calculateIncludedVAT(sumItemPrice, tax);
//                long sumIncludedVAT = includedVAT * itemAmount;
                String sumIncludedVATStr = fmtPrice(sumIncludedVAT);
    ...
    public TablePrinter printOrders(final TablePrinter orderTable, final Collection<Order> orders) {
        ...
        for (Order order : orders) {
            for (OrderItem item : order.getItems()) {
                ...
                totals[1] += calculateIncludedVAT(item.getArticle().getUnitPrice() * item.getUnitsOrdered(), item.getArticle().getTax());
            }
        }
